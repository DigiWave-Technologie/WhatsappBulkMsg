...
const PersonalCampaignServices = require("../services/InternaitionapersonalCampaignServices");
...
exports.createpersonalCampaign = async (req, res) => {
  try {
    ...
    numbersArray = numbersArray.map((number) => {
      if (!number.startsWith(countryCode)) number = `${countryCode}${number}`;
    ...
    // Save campaign record via service
    const result = await PersonalCampaignServices.createpersonalCampaign(
      campaignData
    );

    // Check if button data exists (determines whether to send an interactive message)
    const hasButtonData =
      Boolean(button1Text) ||
    ...
    // Build an array for image media data
    ...
    // Process each phone number: send poll (if provided), then send message/media or interactive message,
    // and finally delay before processing the next number.
    for (const number of numbersArray) {
      // If a poll question is provided, send the poll message first
      if (pollQuestion) {
        await createPoll([number], pollQuestion, pollOptions, QRInstanceid);
      }

      // Then send either an interactive message or standard WhatsApp messages/media
      if (hasButtonData) {
      ...
      // Wait for the specified number of seconds (BetweenMessages) before processing the next number
      if (
        BetweenMessages &&
...
//     const campaignData = await PersonalCampaignServices.getCampaignById(campaignId);
